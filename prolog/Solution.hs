...
data Term= App String [Term] | Var String deriving (Read,Eq)
...
showTerm (Var n) = showString n
showTerm (App n []) = showString n
showTerm (App n list) = showString n . showString "(" . foldl1 (\a b -> a . showString "," . b) (map showTerm list) . showString ")"

instance Show Term where
    show t = showTerm t ""

splitProgramAndGoals :: Program -> (Program, Goals)
splitProgramAndGoals clauseList = foldr (\clause -> \(prg, gl) -> case clause of
                                                                       Clause [] lst -> (prg, lst++gl)
                                                                       _ -> (clause:prg,gl)) ([],[]) clauseList

flattenRawSubs subs = flatten $ Just $ Subs subs

unSubs (Subs s) = s

dfs :: SLDTree -> Maybe Subs
dfs (SLDTree subs [] _) = Just subs
dfs (SLDTree subs _ trees) = foldl (\last -> \tree -> (
    case last of
         Nothing -> (case dfs tree of
                          Nothing -> Nothing
                          Just childSubs -> Just $ Subs (unSubs subs ++ unSubs childSubs))
         _ -> last )
    ) Nothing trees

variablesFromTerm v@(Var _) = [v]
variablesFromTerm (App _ list) = foldr (++) [] (map variablesFromTerm list)

unPred :: AtomicF -> [Term]
unPred (Pred _ lst) = lst

filterInterestingVariables :: [Term] -> Subs -> Subs
filterInterestingVariables vars (Subs list) = Subs $ filter (\(v,_) -> elem v vars) list

solution :: IO ()
solution = do
    input <- getContents
    let (program, goals) = splitProgramAndGoals (parseProgram input)
    let interestingVariables = concat $ map variablesFromTerm $ concat $ map unPred goals
    let tree = sldTree program goals
    let subs = catMaybes [fmap (filterInterestingVariables interestingVariables) $ flatten $ dfs tree]
    print subs
